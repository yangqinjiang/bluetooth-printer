...
(function (global) {
    ...
    ArrayBuffer.prototype.sliceData = function (maxByteCount) {
        ...
        let that = this
        return function () {
            if (that.byteLength <= 0) {
                console.error("打印空数据")
                return new ArrayBuffer(0)
            }

            //获取一部分数据
            let subPackage = that.slice(i, i + maxByteCount <= that.byteLength ? (i + maxByteCount) : that.byteLength);
 ...
 */
    Printer.prototype.SendToBLEPrinter = function (slicer) {
        ...
        // this.SendToPrinter = function (write_buf,i, that._total) {
        ...
        if (typeof this.SendToPrinter !== 'function') {
            throw new Error("SendToPrinter 必须是一个闭包函数,签名是: function (write_buf,i, that._total) , 返回值是Promise")
        }
